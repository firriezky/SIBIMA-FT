...
$mentee_id =$_POST['mentor_id']; 
//harusnya mentor_id variabelnya , tapi karena males langsung capcus
//santai semantic doang kok
...
$queryCheckMentee="SELECT nama,fakultas from mentor where id=$mentee_id";
$sqlCMentee=mysqli_query($conn,$queryCheckMentee);
$rowCMentee=mysqli_fetch_array($sqlCMentee);


$fakultas_mentor=$rowCMentee['fakultas'];
...
$queryCheckFakultas="SELECT nama,fakultas from mentor where id=$mentee_id";
...
$queryCheckDuplicate ="SELECT * FROM presensi where mentor_id ='$mentee_id' AND agenda_id='$agenda_id'";
...
$queryInsert = "insert into presensi (created_at,updated_at,mentor_id,agenda_id,waktu_hadir) values (now(),now(),$mentee_id,$agenda_id,now())";
